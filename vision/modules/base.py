...
class ModuleBase:
    ...
    def normalized(self, coord, mat=None, axis=None):
        if mat is None:
            mat = self._next_images[0]

        def norm(compon, axis):
            return 2 * compon / mat.shape[1] -1

        return (norm(coord[0], 1), norm(coord[1], 0))

    # Converts coord to exact coordinates
    def denormalized(self, coord, mat=None, round=False):
        if mat is None:
            mat = self._next_images[0]

        def denorm(compon, axis=None):
            denormed = (compon + 1) / 2 * mat.shape[1]
            if round:
                denormed = int(denormed)
            return denormed

        return (denorm(coord[0], 1), denorm(coord[1], 0))
