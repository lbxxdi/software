...
class BuoyData:
    ...
    def set_results(self, image_width, image_height, contour_list):
        ...
        self.results.center_x = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[0])
        self.results.center_y = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[1])
        self.results.top_x = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[0])
        self.results.top_y = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[1] - self.contour.radius / 2)
        self.results.bottom_y = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[1] + self.contour.radius)
        self.results.r_side_x = int(1 / constants.DOWNSCALE_RATIO) * int(self.contour.center[0] + self.contour.radius)
...
class Buoys(ModuleBase):
    ...
    def process(self, mat):
        ...
        mat = cv2.resize(mat, (int(mat.shape[1] * constants.DOWNSCALE_RATIO), int(mat.shape[0] * constants.DOWNSCALE_RATIO)))
        ...
        def morph_and_find_contours(channels_and_names, channel_set_name,
                                    debug=False):
            """
            ...
            channels_and_bounds = starmap(
                ...
                channels_and_names)
            thresheds = starmap(cv2.inRange, channels_and_bounds)
            ...
            _, contours, _ = cv2.findContours(morphed.copy(),
                                              cv2.RETR_EXTERNAL,
                                              cv2.CHAIN_APPROX_SIMPLE)

            return morphed, contours
        ...
        red_morphed, red_contours = morph_and_find_contours(
            red_channels_and_names,
            "red",
        ...
        yellow_morphed, yellow_contours = morph_and_find_contours(
            yellow_channels_and_names,
            "yellow",
        ...
        green_morphed, _ = morph_and_find_contours(
            green_channels_and_names,
            "green",
